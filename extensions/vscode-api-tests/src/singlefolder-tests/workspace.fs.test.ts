...
function textToData(text: string): Uint8Array {
...
suite('vscode API - workspace-fs', () => {
	...
	test('fs.write/stat/read/delete', async function () {
		...
		await vscode.workspace.fs.writeFile(uri, textToData('HELLO'));
	...
	test('fs.delete folder', async function () {
		...
		await vscode.workspace.fs.writeFile(file, textToData('FOO'));
	...
	test('fs.createFolder creates recursively', async function () {
		...
		await vscode.workspace.fs.writeFile(file, textToData('Hello World'));
