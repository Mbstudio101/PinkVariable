...
export class McpRegistryInputStorage extends Disposable {
	...
	private async _unsealSecrets(): Promise<Record<string, IResolvedValue>> {
		...
		try {
			...
			// Create proper ArrayBuffer instances (not SharedArrayBuffer) for crypto operations
			// VSBuffer.buffer is Uint8Array, and Uint8Array.buffer can be SharedArrayBuffer
			const ivArrayBuffer = new ArrayBuffer(iv.buffer.byteLength);
			new Uint8Array(ivArrayBuffer).set(iv.buffer);
			const encryptedArrayBuffer = new ArrayBuffer(encrypted.buffer.byteLength);
			new Uint8Array(encryptedArrayBuffer).set(encrypted.buffer);

			const decrypted = await crypto.subtle.decrypt(
				{ name: MCP_ENCRYPTION_KEY_ALGORITHM, iv: ivArrayBuffer },
				key,
