...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/PinkVariable
	...
	async initialize() {
		...
		const { commit, version, pinkVariableVersion, release, quality } = this._productService
		...
		this._initProperties = {
			...
			pinkVariableVersion: pinkVariableVersion || version,
			release,
			os,
			quality,
		...
		console.log('User is opted out of basic PinkVariable metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('PinkVariable posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
